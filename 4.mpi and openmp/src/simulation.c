...
void simulation(int processId, int numProcesses, int numThreads, DivisionStructure division, int numIterations, int numRows, int numCols, TimeResultRef timeResult){
	...
	MatrixSubTaskRef threadsSubtasks = NULL;
	PlateRef currentPlate = NULL, oldPlate = NULL, switchPlate = NULL;
	int numVirutalRows, numVirutalCols, iteration, i, j, t, numAssignedRows, numAssignedCols;
	...
	// calculate thread divisions
	numAssignedRows = 1 + (subtask.toRow - subtask.fromRow);
	numAssignedCols = 1 + (subtask.toCol - subtask.fromCol);
	threadsSubtasks = buildMatrixDivisions(numAssignedRows, numAssignedCols, numThreads, division);
	
	// adjust assignments
	for(t=0; t<numThreads; t++){
		threadsSubtasks[t].toRow += subtask.fromRow;
		threadsSubtasks[t].toCol += subtask.fromCol;
		threadsSubtasks[t].fromRow += subtask.fromRow;
		threadsSubtasks[t].fromCol += subtask.fromCol;
	}
	...
	#pragma omp parallel num_threads(numThreads)
	{
		...
		for(iteration=0; iteration<numIterations; iteration++){
			...
			#pragma omp for
			for(t=0; t < numThreads; t++){
				loopBehaviour(oldPlate, currentPlate, threadsSubtasks[t]);
			}
