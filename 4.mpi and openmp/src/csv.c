...
void addCsvEntry(int simulationType, int divisionType, int numIterations, int numProcesses, int numThreads, int numRows, int numCols, TimeResult result){
	int i;
	FILE * f = NULL;
	double meanIteration = 0, totalIterationsTime = 0;
	...
	if(access( CSV_FILE, F_OK ) == -1 ){
		...
		// write header
		fprintf(f,"TIME;ENTRY_TYPE;SIMULATION_TYPE;DIVISION_TYPE;PROCESSES;THREADS;ITERATIONS;ROWS;COLUMNS;TOTAL_TIME;TOTAL_CALCULATION_TIME;MEAN_ITERATION_TIME");
	}else{
		// open file
		f = fopen(CSV_FILE,"a+");
		if(NULL == f){
	...
	// print registry 
	char simulationTypeChar = 'P'; 
	char divisionTypeChar = (divisionType == DIVISION_BY_ROWS) ? 'R' : ( (divisionType == DIVISION_BY_COLS)  ? 'C' : 'M' ); 
    fprintf(f, "\n%s;%s;%c;%c;%d;%d;%d;%d;%d;%g;%g;%g", getDateAndTime(), ENTRY_TYPE, simulationTypeChar, divisionTypeChar, numProcesses, numThreads, numIterations, numRows, numCols, (double)result.totaltime, (double)totalIterationsTime, (double)meanIteration);
