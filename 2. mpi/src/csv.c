...
void addCsvEntry(int simulationType, int divisionType, int numIterations, int numThreads, int numRows, int numCols, TimeResult result){
	...
	// print registry 
	char divisionTypeChar = 'M';
	char simulationTypeChar = 'P'; 
    fprintf(f, "\n%s;%s;%c;%c;%d;%d;%d;%d;%g;%g;%g", getDateAndTime(), ENTRY_TYPE, simulationTypeChar, divisionTypeChar, numThreads, numIterations, numRows, numCols, (double)result.totaltime, (double)totalIterationsTime, (double)meanIteration);
