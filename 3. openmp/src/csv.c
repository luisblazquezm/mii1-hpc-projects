...
void addCsvEntry(int simulationType, int divisionType, int numIterations, int numThreads, int numRows, int numCols, TimeResult result){
    ...
    fprintf(f, "\n%s;%s;%c;%c;%d;%d;%d;%d;%d;%d;%f", getDateAndTime(), ENTRY_TYPE, simulationTypeChar, divisionTypeChar, numThreads, numIterations, numRows, numCols, result.totaltime, totalIterationsTime, meanIteration);
