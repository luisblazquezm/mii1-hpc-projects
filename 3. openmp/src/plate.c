...
PlateRef createPlate(int numRows, int numCols){
	...
	plate->values = (Value **) calloc(plate->rows, sizeof(Value *));
	for(i=0; i < plate->rows; i++){
		plate->values[i] = (Value *) calloc(plate->cols, sizeof(Value));
		if(plate->values[i] == NULL){
